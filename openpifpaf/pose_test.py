...
import logging
from openpifpaf import Predictor, CHECKPOINT_URLS
import openpifpaf
...
class PoseEstimator:
    def __init__(self, model_type, version):
        ...
        self.models = {
            ...
            "openpifpaf": (self.openpifpaf_initialize, self.openpifpaf_process_image)
        }
        self.model_type = model_type
    ...
    def draw_keypoints(self, poses, scores, frame):
        ...
        if (len(poses) == 0):
            cv2.putText(frame, "No poses detected", (10, 10),
        ...
        for i in range(len(poses)):
            ...
            if scores[i] > 0.5:
                ...
                for p in range(keypoints.shape[0]):
                    # draw the keypoints
                    cv2.putText(frame, f"{p}", (int(keypoints[p, 0]), int(keypoints[p, 1])), cv2.FONT_HERSHEY_SIMPLEX,
                                0.5, (0, 0, 0), 1)
                ...
                for ie, e in enumerate(self.edges):
                    ...
                    if e[0] < keypoints.shape[0] and e[1] < keypoints.shape[0]:
    ...
    def mediapipe_initialize(self):
        ...
        base_options = mp_python.BaseOptions(
            model_asset_path=model_path, delegate=mp_python.BaseOptions.Delegate.GPU)
    ...
    def openpifpaf_initialize(self):
        if self.version not in [c for c, u in CHECKPOINT_URLS.items() if isinstance(u, str)]:
            print("Invalid version")
            logger.error("Invalid version")
            return
        self.model = Predictor(checkpoint=self.version, head_metas=[openpifpaf.headmeta.Tcaf])
    def openpifpaf_process_image(self, frame, frame_width, frame_height, timestamp):
        frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

        predictions, gt_anns, meta = self.model.numpy_image(frame_rgb)

        keypoints = np.array([prediction.data for prediction in predictions])
...
def main(video_folders, out_folder, model_type, version="", input_formats=["mp4", "avi"]):
    ...
    if len(files) > 0:
        logger.info(
            f"Average time/frame for {model_type} {version}:\t{tim/frame_number:.4f}s, {frame_number/tim}fps")
...
for model in openpifpaf_models:
    main([r'C:\Users\Test\Desktop\SafetyVideoDetection\samples\video\fifty_ways\test', r'C:\Users\Test\Desktop\SafetyVideoDetection\samples\video\cauca\test'], "samples\\out\\haha",
         "openpifpaf", version=model)

# main(r'C:\Users\Test\Desktop\SafetyVideoDetection\samples\video\fifty_ways\test', "samples\\out",
