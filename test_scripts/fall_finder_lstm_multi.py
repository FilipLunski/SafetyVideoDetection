...
def main(video_folder, out_folder="", input_format="mp4", seconds_before=2, seconds_after=2, threshold=0.5, lstm_timestamps=30, device="cuda"):
    global labels
    ...
    for file in files:
        ...
        (f, t) = processFile(file, out_folder, seconds_before=2, seconds_after=2,
                             threshold=threshold, lstm_timestamps=lstm_timestamps, device=device)
        time_all += t
        frames_all += f
...
def processFile(file, out_folder, seconds_before, seconds_after, threshold, lstm_timestamps, device):
