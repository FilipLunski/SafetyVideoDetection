...
def processFile(file, out_folder, seconds_before, seconds_after, treshold, lstm_timestamps, device):
    ...
    fall_finder = FallDetector(pose_model, fall_model, lstm_timestamps, frame_width,
                             frame_height, frame_rate, output_filename, output_annotated_filename, device=device)
...
def main(video_folder, out_folder="", input_format="mp4", seconds_before=2, seconds_after=2, treshold=0.5, lstm_timestamps=50, device="cuda"):
    ...
    global fall_model  
    fall_model = KeypointClassifierLSTM.load_from_checkpoint(
        "./logs_m/lstm_None_1_128_64_0.3_0.3/version_0/checkpoints/epoch=399-step=2000.ckpt")
...
main(r'samples\video\fifty_ways\test', "samples\\out\\lstm", "mp4", 3, 2, 0.5, device="cpu")
